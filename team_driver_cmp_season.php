...
$mask = "| %-18s | %-12s | %-10s | %12s | %9s |\n";
print "<pre>";
printf($mask, "------------------", "------------", "----------", "------------", "---------");
printf($mask, "driver name", "driver code", "driver num", "total points", "num races");
printf($mask, "------------------", "------------", "----------", "------------", "---------");
while($row = mysqli_fetch_array($result, MYSQLI_BOTH))
{
    printf($mask, "$row[driver_name]", "$row[driver_code]", "$row[driver_num]", "$row[total_points]", "$row[num_races]");
}
printf($mask, "------------------", "------------", "----------", "------------", "---------");
print "</pre>";
