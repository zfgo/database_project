...
\underline{underlined}, and foreign keys are \textit{italicized}. If an
attribute is a primary key and a foreign key, then it is
\underline{\textit{underlined and italicized}}. Not
null requirements are indicated. Primary keys and foreign keys are
always not null.

\begin{itemize}
    \item \textbf{TEAM}: \underline{team\_id}, name, power\_unit, chasis
    \begin{itemize}
        \item not null: power\_unit, chasis
    \end{itemize}

    \item \textbf{DRIVER}: \underline{driver\_id},
    \textit{team\_team\_id}, code, number, fname, lname, dob
    \begin{itemize}
        \item not null: code, number, fname, lname
    \end{itemize}

    \item \textbf{RESULT}: \underline{\textit{race\_race\_id}},
    \underline{\textit{race\_season\_year}}, \underline{\textit{driver\_driver\_id}},
    \textit{points\_position}, fastest\_lap

    \item \textbf{POINTS}: \underline{position}, points
    \begin{itemize}
        \item not null: points
    \end{itemize}

    \item \textbf{RACE}: \underline{race\_id},
    \underline{\textit{season\_year}}, \textit{circuit\_circuit\_id},
    date, name 
    \begin{itemize}
        \item not null: date, name
    \end{itemize}

    \item \textbf{SEASON}: \underline{year}

    \item \textbf{CIRCUIT}: \underline{circuit\_id}, name, lat, long,
    length, n\_laps
    \begin{itemize}
